...
export function SettingsClient({ profile, email }: SettingsClientProps) {
  // Profil state — initialised from profile prop
  ...
  // Powiadomienia state — initialised from profile prop
  const [emailEnabled, setEmailEnabled] = useState(
    (profile?.notification_email as boolean) ?? true
  );
  const [pushEnabled, setPushEnabled] = useState(
    (profile?.notification_push as boolean) ?? false
  );
  const [smsEnabled, setSmsEnabled] = useState(
    (profile?.notification_sms as boolean) ?? false
  );
  ...
  // Preferencje state — initialised from profile prop
  const savedRegions = (profile?.preferred_regions as string[]) ?? [];
  const savedCpv = (profile?.preferred_cpv_codes as string[]) ?? [];
  const savedKfs = (profile?.kfs_priorities as string[]) ?? [];

  const [regions, setRegions] = useState<Record<string, boolean>>(
    Object.fromEntries(VOIVODESHIPS.map((v) => [v, savedRegions.includes(v)]))
  );
  const [cpvGroups, setCpvGroups] = useState<Record<string, boolean>>(
    Object.fromEntries(CPV_GROUPS.map((g) => [g, savedCpv.includes(g)]))
  );
  const [kfsPriorities, setKfsPriorities] = useState<Record<string, boolean>>(
    Object.fromEntries(KFS_PRIORITIES.map((p) => [p, savedKfs.includes(p)]))
  );
  const [budgetMin, setBudgetMin] = useState(
    (profile?.budget_min as number | null)?.toString() ?? ""
  );
  const [budgetMax, setBudgetMax] = useState(
    (profile?.budget_max as number | null)?.toString() ?? ""
  );
  ...
  async function handleSaveProfile() {
    ...
    try {
      await updateProfile({ full_name: fullName, company_name: companyName, phone });
      showMsg(setProfileMsg, "success", "Zapisano");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Błąd zapisu";
      showMsg(setProfileMsg, "error", message);
    } finally {
  ...
  async function handleSaveNotifications() {
    ...
    try {
      ...
      showMsg(setNotifMsg, "success", "Zapisano");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Błąd zapisu";
      showMsg(setNotifMsg, "error", message);
    } finally {
  ...
  async function handleSavePreferences() {
    ...
    try {
      await updateSearchPreferences({
        preferred_regions: VOIVODESHIPS.filter((v) => regions[v]),
        preferred_cpv_codes: CPV_GROUPS.filter((g) => cpvGroups[g]),
        kfs_priorities: KFS_PRIORITIES.filter((p) => kfsPriorities[p]),
        budget_min: budgetMin !== "" ? Number(budgetMin) : null,
        budget_max: budgetMax !== "" ? Number(budgetMax) : null,
      });
      showMsg(setPrefsMsg, "success", "Zapisano");
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Błąd zapisu";
      showMsg(setPrefsMsg, "error", message);
    } finally {
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3">
              {notifMsg && (
                <span
                  className={`text-sm ${
                    notifMsg.type === "success" ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {notifMsg.text}
                </span>
              ...
              >
                <SaveIcon />
                {isSavingNotif ? "Zapisywanie…" : "Zapisz ustawienia"}
              </Button>
            </div>
          </div>
        </TabsContent>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-3">
              {prefsMsg && (
                <span
                  className={`text-sm ${
                    prefsMsg.type === "success" ? "text-green-600" : "text-red-600"
                  }`}
                >
                  {prefsMsg.text}
                </span>
              ...
              >
                <SaveIcon />
                {isSavingPrefs ? "Zapisywanie…" : "Zapisz preferencje"}
              </Button>
            </div>
          </div>
        </TabsContent>
